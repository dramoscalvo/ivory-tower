...
function createMinimalEntity(id: string, name: string, type: 'class' | 'interface' | 'abstract-class' = 'class') {
  return { id, name, type }
}
...
describe('calculateHierarchicalLayout', () => {
  ...
  it('interface + implementing class: interface above class', () => {
    const diagram = createDiagram(
      [createMinimalEntity('impl', 'MyClass'), createMinimalEntity('iface', 'MyInterface', 'interface')],
